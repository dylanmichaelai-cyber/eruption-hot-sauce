...
Fetches recent video comments, analyzes them with Claude, and emails a report.

Usage:
    python scripts/youtube_comment_summary.py

Schedule with cron (weekly, Mondays at 8am):
    0 8 * * 1 cd /path/to/eruption-hot-sauce && python scripts/youtube_comment_summary.py
"""
...
def get_recent_videos(youtube, channel_id: str, days_back: int, max_videos: int) -> list[dict]:
...
def build_html_email(analysis: str, video_data: list[dict], channel_name: str, days: int) -> str:
...
<div class="wrap">
  ...
  <div class="footer">
    ...
    Re-run anytime: <code>python scripts/youtube_comment_summary.py</code>
  </div>
</div>
</body>
</html>"""
...
def send_email(sender: str, password: str, recipient: str, subject: str, html: str):
    msg             = MIMEMultipart("alternative")
    msg["Subject"]  = subject
    msg["From"]     = sender
    msg["To"]       = recipient
    msg.attach(MIMEText("View this report in an HTML-capable email client.", "plain"))
...
def main():
    ...
    print(f"[1/4] Fetching videos from the last {DAYS_TO_LOOK_BACK} days...")
    ...
    print(f"[2/4] Found {len(videos)} video(s) on '{channel_name}'. Fetching comments...")
    video_data = []
    for v in videos:
        print(f"  → {v['title']}")
    ...
    html    = build_html_email(analysis, video_data, channel_name, DAYS_TO_LOOK_BACK)
    subject = f"ERUPTION — YouTube Comment Report · {datetime.now().strftime('%b %d, %Y')}"
    send_email(EMAIL_SENDER, EMAIL_PASSWORD, EMAIL_RECIPIENT, subject, html)

    print(f"Done. Report sent to {EMAIL_RECIPIENT}.")
